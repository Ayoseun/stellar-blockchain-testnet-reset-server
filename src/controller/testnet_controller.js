...
import { sendEvent } from '../stream/response_stream.js'
const optsopt = {
  allowHttp: true,
}
const server = new Horizon.Server(process.env.HORIZON, optsopt)
export const TestNetController = async (req, res) => {
  res.setHeader('Content-Type', 'text/event-stream')
  res.setHeader('Cache-Control', 'no-cache')
  res.setHeader('Connection', 'keep-alive')
  sendEvent(res, 'message', 'Setting up accounts on testnet')

  console.log('Setting up accounts')
  try {
    await axios.get(process.env.BOTURL + '/?addr=' + process.env.ISSUER_PUB)
    console.log('Issuer account setup done')
    sendEvent(res, 'message', 'Issuer account setup done')
  } catch (error) {
    if (error.response.data.status == 400) {
      console.log('Issuer account setup failed')
      sendEvent(res, 'message', 'Issuer account setup failed')
    } else {
      console.log('Issuer account setup failed')
      sendEvent(res, 'message', 'Issuer account setup failed')
    }
  ...
  try {
    ...
    sendEvent(res, 'message', 'distributor account setup done')
  } catch (error) {
    if (error.response.data.status == 400) {
     
      sendEvent(res, 'message', 'distributor account setup failed')
    } else {
  ...
  try {
    ...
    sendEvent(res, 'message', 'Pool account setup done')
  } catch (error) {
    if (error.response.data.status == 400) {
      console.log('pool account setup done')
      sendEvent(res, 'message', 'Pool account setup failed')
    } else {
  ...
  try {
    ...
    sendEvent(res, 'message', 'Creating asset trustline')
  } catch (error) {
    console.log(error)
    sendEvent(res, 'message', 'Creating asset trustline failed')
  }
  ...
  try {
    ...
    await server.submitTransaction(transaction, memocheck)
    sendEvent(res, 'message', 'Setting pool trustline')
  } catch (error) {
    console.log(error)
  }
  ...
  try {
    ...
    const assets = new Asset(process.env.ASSET, process.env.ISSUER_PUB)
    const payobj = {
      destination: process.env.DIST_PUB,
      asset: assets,
      amount: '10000000',
    }
    const payobj2 = {
      destination: process.env.POOL_PUB,
      asset: assets,
      amount: '50000',
    }
    ...
    await server.submitTransaction(transaction, memocheck)
    sendEvent(res, 'message', 'Building asset issuance')
  } catch (error) {
    console.log(error)
  }
  ...
  try {
    ...
    await server.submitTransaction(transaction, memocheck)
    sendEvent(res, 'message', 'Creating USDO<>XLM pool')
  } catch (error) {
    sendEvent(res, 'message', 'Setting up error')
    console.log(error)
  }
  ...
  try {
    ...
    await server.submitTransaction(transaction, memocheck)
    sendEvent(res, 'message', 'Creating USDO<>XLM pool Initial Deposit')
    sendEvent(res, 'message', 'All done')
  } catch (error) {
    console.log(error)
    sendEvent(res, 'message', 'Creating USDO<>XLM pool Initial Deposit Failed')
  }
}
